...
const EditImage1 = ({ id, image }) => {
  ...
  const handleSubmitNewImage = (e) => {
    ...
    if (newImage) {
      ...
      axios
        .put(
          `http://localhost:8000/images/update/image1/${id}`,
          formData,
          config
        )
        .then(({ data }) => {
  ...
  const handleSubmit = async () => {
    await axios
      .put(`http://localhost:8000/images/delete/image1/${id}`)
      .then(({ data }) => {
        if (data.error) setError(data.error);
        else {
          setResult(true);
  ...
  return (
    <div className="h-1/4 bg-neutral-100 px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
      <dt className="text-l font-medium text-gray-500 ">Image principale</dt>
          ...
          <div className="w-full">
            <div className="flex justify-center">
              {image === null ? (
                ...
                <img
                  className="w-full md:h-auto object-cover md:w-2/6 rounded-t-lg md:rounded-none md:rounded-l-lg"
                  src={image}
                  alt=""
                />
              )}
