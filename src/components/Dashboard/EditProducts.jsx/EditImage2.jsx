...
const EditImage2 = ({ id, image }) => {
  ...
  const handleSubmitNewImage = (e) => {
    ...
    if (newImage) {
      ...
      axios
        .put(
          `http://localhost:8000/images/update/image2/${id}`,
          formData,
          config
        )
        .then(({ data }) => {
  ...
  const handleSubmit = async () => {
    await axios
      .put(`http://localhost:8000/images/delete/image2/${id}`)
      .then(({ data }) => {
        if (data.error) setError(data.error);
        else {
          setResult(true);
  ...
  return (
    <div className="h-1/4 bg-white px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
      <dt className="text-l font-medium text-gray-500 ">Image 2</dt>
